...
const Navbar = () => {
  ...
  return (
    <>
        <header className={
            !navState ? 'absolute top-7 left-0 right-0 opacity-100 z-50' : 'fixed flex items-center justify-center opacity-100 z-[200] blur-effect-theme top-0 left-0 right-0 h-[9vh]'
        }>
            <nav className='flex items-center justify-between nike-container'>
                <div className='flex items-center'>
                    <img
                        src={logo} alt='logo/img' className={`w-16 h-auto ${navState && "filter brightness-0"}`}
                    />
                </div>
